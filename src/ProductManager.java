...
public class ProductManager {
	...
	public double calculateAveragePriceInCategory(String category) {
		// String olarak verilen category'e ait olan ürünlerin fiyatlarının
		// ortalamasını, yoksa 0.0 döndüren metodu yazın
		// tip: OptionalDouble kullanımını inceleyin.

		List<Product> productsCategoried = products.values().stream()
				.filter(product -> product.getCategory().equals(category)).toList();

		double averageProducts = productsCategoried.stream().mapToDouble(Product::getPrice).average().orElse(0.0);
	...
	public Map<String, Double> getCategoryPriceSum() {
		...
		// fiyatını stream ile hesaplayıp döndüren metodu yazın
		// örn:
		// category-1 105.2
		// category-2 45.0

		Map<String, Double> groupCategoryPrices = products.values().stream()
				.collect(Collectors.groupingBy(Product::getCategory, Collectors.summingDouble((Product::getPrice))));

		return groupCategoryPrices;
